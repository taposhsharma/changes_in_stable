...
form.addEventListener("submit", function (event) {
  ...
  const ignoredDeptDetails = {
    hospitalId: hospitalIdValue,
    key: keyValue,
    value: valueValue,
  };

  console.log(ignoredDeptDetails);

  // Making the POST request
  fetch("http://localhost:3006/frontend/addignoredDepts", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(ignoredDeptDetails),
  })
    .then((response) => {
      ...
      if (response.ok) {
        Swal.fire({
          icon: "success",
          title: "Success!",
          text: "ignoredDepts added successfully",
        ...
        Swal.fire({
          icon: "error",
          title: "Error!",
          text: "ignoredDepts with this id already exists",
          showConfirmButton: false, // Remove the confirm button
          timer: 2000, // Automatically close after 1.5 seconds
        });
        console.error("ignoredDepts with this id already exists");
      }
    })
    .catch((error) => {
      Swal.fire({
      ...
      console.error("ignoredDepts with this id already exists! ", error);
    });
});
