function setPreviousPage() {
  ...
  form.addEventListener("submit", function (event) {
    ...
    const orgDeptMapDetails = {
      hospitalId: hospitalIdValue,
      key: keyValue,
      value: valueValue
    };
  
    console.log(orgDeptMapDetails);
  
    // Making the POST request
    fetch("http://localhost:3006/frontend/addorgDeptMap", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(orgDeptMapDetails),
    })
      .then((response) => {
        ...
        if (response.ok) {
          Swal.fire({
            icon: "success",
            title: "Success!",
            text: "orgDeptMap added successfully",
          ...
          // You can perform further actions here after the request is successful
            ...
            Swal.fire({
                icon: "error",
                title: "Error!",
                text: "orgDeptMap with this id already exists",
                showConfirmButton: false, // Remove the confirm button
                timer: 2000, // Automatically close after 1.5 seconds
              });
          console.error("orgDeptMap with this id already exists");
        }
      })
      .catch((error) => {
        Swal.fire({
            icon: "error",
            title: "Error!",
            text: "Error in adding orgDeptMap!",
            showConfirmButton: false, // Remove the confirm button
            timer: 2000, // Automatically close after 1.5 seconds
          });
        console.error("Error in adding orgDeptMap!", error);
      });
  });
